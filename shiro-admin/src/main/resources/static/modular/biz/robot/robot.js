...
Robot.showData = function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            Robot.appendDialog(true, "me", message);
            Robot.appendDialog(false, "机器人", result.data);
            $(".direct-chat form input").each(function () {
                $(this).val("");
            });
        }
...
$(function () {
    ...
    content += "<br/>回复[帮助]查看所有可用指令。";
    // 添加公众号的开场白
    Robot.appendDialog(false, "机器人", content);

    $("#biz").attr("class", "active");
    $("#robot").attr("class", "active");
});